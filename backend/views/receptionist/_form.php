...
/** @var common\models\TblReceptionist $model */
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="alert alert-info">
                ...
                A user account will be automatically created for this receptionist.<br>
                <strong>Username format:</strong> <code>rec.firstname.lastname</code><br>
                <strong>Password format:</strong> <code>Lastname@emailusername</code> (e.g., email <code>juana.santos@hospital.com</code> → password <code>Santos@juanasantos</code>)
            </div>
            <?php endif; ?>
            ...
            <div class="row mt-3">
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="form-label">Generated Username Preview</label>
                        <input type="text" id="username-preview" class="form-control" readonly 
                               placeholder="rec.firstname.lastname">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="form-label">Generated Password Preview</label>
                        <input type="text" id="password-preview" class="form-control" readonly 
                               placeholder="Lastname@emailusername">
                    </div>
                </div>
            </div>
            <?php endif; ?>
...
$this->registerJs("
    ...
    function updateCredentialPreview() {
        ...
        var username = 'rec.' + cleanFirst + '.' + cleanLast;
        $('#username-preview').val(username);
        
        // Generate password: Lastname@emailusername
        var cleanLastCapitalized = lastname.charAt(0).toUpperCase() + lastname.slice(1).toLowerCase().replace(/[^a-zA-Z]/g, '');
    ...
    $(document).ready(function() {
        ...
        if (initialCountryCode) $('#recep-country_code').trigger('change');
        var initialEmail = $('#recep-email').val();
        if (initialEmail) $('#recep-email').trigger('change');
        if ($('#recep-lastname').val()) updateCredentialPreview();
    });
");
?>